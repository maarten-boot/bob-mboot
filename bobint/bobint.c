...
/* call frame dispatch */
static void
CallRestore(BobInterpreter *c);

static BobValue *
CallCopy(BobInterpreter *c, BobFrame *frame);
...
static void
BlockRestore(BobInterpreter *c);

static BobValue *
BlockCopy(BobInterpreter *c, BobFrame *frame);
...
/* prototypes */
static BobValue
ExecuteCall(BobInterpreter *c, BobValue fun, int argc, va_list ap);

static void
Execute(BobInterpreter *c);

static void
UnaryOp(BobInterpreter *c, int op);

static void
BinaryOp(BobInterpreter *c, int op);

static int
Send(BobInterpreter *c, FrameDispatch *d, int argc);

static int
Call(BobInterpreter *c, FrameDispatch *d, int argc);

static void
PushFrame(BobInterpreter *c, int size);

static BobValue
UnstackEnv(BobInterpreter *c, BobValue env);

static void
BadOpcode(BobInterpreter *c, int opcode);

static int
CompareObjects(BobInterpreter *c, BobValue obj1, BobValue obj2);

static int
CompareStrings(BobValue str1, BobValue str2);

static BobValue
ConcatenateStrings(BobInterpreter *c, BobValue str1, BobValue str2);
