...
/* Vector methods */
static BobCMethod methods[] = {BobMethodEntry(0, 0)};

/* Vector properties */
static BobVPMethod properties[] = {BobVPMethodEntry("printName", BIF_printName, 0), BobVPMethodEntry(0, 0, 0)};
...
BobDispatch BobSymbolDispatch = {
        "Symbol", &BobSymbolDispatch, GetSymbolProperty, SetSymbolProperty, BobDefaultNewInstance, SymbolPrint,
        SymbolSize, SymbolCopy, BobDefaultScan, SymbolHash
};
...
BobValue BobInternString(BobInterpreter *c, unsigned char *printName, int length) {
    ...
    for (; p != c->nilValue; p = BobPropertyNext(p)) {
        ...
        if (length == BobSymbolPrintNameLength(sym) && memcmp(printName, BobSymbolPrintName(sym), length) == 0) {
            return sym;
        }
    }
