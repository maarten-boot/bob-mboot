...
static int IntegerWrite(BobInterpreter *c, BobValue obj, BobStream *s) {
    return WriteLong(BobTypeTag(obj), s) && WriteLong((long) BobIntegerValue(obj), s);
}
...
static int FloatWrite(BobInterpreter *c, BobValue obj, BobStream *s) {
    return WriteLong(BobTypeTag(obj), s) && WriteFloat(BobFloatValue(obj), s);
}
...
static int WritePointer(BobInterpreter *c, BobValue p, BobStream *s) {
    return BobPointerP(p) ? WriteLong((long) ((unsigned char *) p - c->newSpace->base), s) : WriteLong((long) p, s);
}
