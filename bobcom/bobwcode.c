...
static int WriteHeader(BobInterpreter *c, BobStream *s) {
    return BobStreamPutC('B', s) != BobStreamEOF && BobStreamPutC('O', s) != BobStreamEOF &&
           BobStreamPutC('B', s) != BobStreamEOF && BobStreamPutC('O', s) != BobStreamEOF &&
...
static int WriteCodeValue(BobInterpreter *c, BobValue v, BobStream *s) {
    return BobStreamPutC(BobFaslTagCode, s) != BobStreamEOF && WriteVector(c, v, s);
}

/* WriteVectorValue - write a vector value */
static int WriteVectorValue(BobInterpreter *c, BobValue v, BobStream *s) {
    return BobStreamPutC(BobFaslTagVector, s) != BobStreamEOF && WriteVector(c, v, s);
}
...
static int WriteObjectValue(BobInterpreter *c, BobValue v, BobStream *s) {
    ...
    if (BobStreamPutC(BobFaslTagObject, s) == BobStreamEOF || !WriteValue(c, c->nilValue, s) /* class symbol */
    ...
    if (BobVectorP(p)) {
        ...
        for (i = 0; i < size; ++i) {
            ...
            for (; pp != c->nilValue; pp = BobPropertyNext(pp)) {
                if (!WriteValue(c, BobPropertyTag(pp), s) || !WriteValue(c, BobPropertyValue(pp), s)) {
                    return FALSE;
                }
            }
        }
        ...
        for (; p != c->nilValue; p = BobPropertyNext(p)) {
            if (!WriteValue(c, BobPropertyTag(p), s) || !WriteValue(c, BobPropertyValue(p), s)) {
                return FALSE;
            }
        }
    }
...
static int WriteStringValue(BobInterpreter *c, BobValue v, BobStream *s) {
    return BobStreamPutC(BobFaslTagString, s) != BobStreamEOF && WriteString(BobStringAddress(v), BobStringSize(v), s);
}

/* WriteIntegerValue - write an integer value */
static int WriteIntegerValue(BobInterpreter *c, BobValue v, BobStream *s) {
    return BobStreamPutC(BobFaslTagInteger, s) != BobStreamEOF && WriteInteger(BobIntegerValue(v), s);
}
...
static int WriteInteger(BobIntegerType n, BobStream *s) {
    return BobStreamPutC((int) (n >> 24), s) != BobStreamEOF && BobStreamPutC((int) (n >> 16), s) != BobStreamEOF &&
           BobStreamPutC((int) (n >> 8), s) != BobStreamEOF && BobStreamPutC((int) (n), s) != BobStreamEOF;
}
