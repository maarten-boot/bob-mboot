...
static int getch(BobCompiler *c) {
    ...
    while (*c->linePtr == '\0') {
        ...
        if (c->atEOF) {
            ...
            /* read the line */
            for (c->linePtr = c->line; (ch = BobStreamGetC(c->input)) != BobStreamEOF && ch != '\n';) {
                *c->linePtr++ = ch;
            }
            *c->linePtr++ = '\n';
            *c->linePtr = '\0';
